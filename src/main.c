...
#endif
static uint8_t __aligned(4) tmsScanlineBuffer[VIRTUAL_PIXELS_X / 2];// TMS9918_PIXELS_X ];
...
static void __time_critical_func(tmsScanline)(uint16_t y, VgaParams* params, uint16_t* pixels)
{
  ...
  if (y < vBorder || y >= (tms9918->mode == TMS_MODE_TEXT80_8 ? (vBorder+384) : (vBorder + TMS9918_PIXELS_Y)))
  {
    ...
    if (frameCount < 600)
    {
      ...
      if (y < (tms9918->mode == TMS_MODE_TEXT80_8 ? (480 -1) : (VIRTUAL_PIXELS_Y - 1)))
      {
  ...
  int tmsX = 0;
  if (tms9918->mode == TMS_MODE_TEXT80 || tms9918->mode == TMS_MODE_TEXT80_8)
  {
    for (int x = hBorder; x < hBorder + (tms9918->mode == TMS_MODE_TEXT80_8 ? VIRTUAL_PIXELS_X : (TMS9918_PIXELS_X * 2)); x += 2, ++tmsX)
    {
